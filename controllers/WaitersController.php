...
class WaitersController extends BaseController {
    ...
    public function tableOrders($tableId = null) {
        $this->requireRole([ROLE_WAITER]);
        
        $user = $this->getCurrentUser();
        $waiter = $this->waiterModel->findBy('user_id', $user['id']);
        
        if (!$waiter) {
            $this->redirect('dashboard', 'error', 'Usuario no encontrado como mesero');
            return;
        }
        
        if ($tableId) {
            // Show orders for specific table
            $table = $this->tableModel->find($tableId);
            if (!$table) {
                $this->redirect('waiters/tableOrders', 'error', 'Mesa no encontrada');
                return;
            }
            
            // Check if waiter has permission to view this table
            if ($table['waiter_id'] && $table['waiter_id'] != $waiter['id']) {
                $this->redirect('waiters/tableOrders', 'error', 'No tienes permisos para ver los pedidos de esta mesa');
                return;
            }
            
            $orderModel = new Order();
            $orders = $orderModel->getOrdersWithDetails(['table_id' => $tableId]);
            
            $this->view('waiters/table_orders_detail', [
                'table' => $table,
                'orders' => $orders,
                'waiter' => $waiter,
                'user' => $user
            ...
            // Show all tables assigned to this waiter
            $tables = $this->tableModel->getWaiterTables($waiter['id']);
            
            // Get available tables that waiter can access
            $availableTables = $this->tableModel->getAvailableTablesForWaiter($waiter['id']);
            
            // Get summary of orders by table
            $orderModel = new Order();
            $tableOrderSummary = [];
            
            foreach ($tables as $table) {
                $todayOrders = $orderModel->getOrdersWithDetails([
                    'table_id' => $table['id'],
                    'date' => date('Y-m-d')
                ]);
                
                $tableOrderSummary[$table['id']] = [
                    'table' => $table,
                    'orders' => $todayOrders,
                    'pending_count' => count(array_filter($todayOrders, function($o) { 
                        return in_array($o['status'], [ORDER_PENDING, ORDER_PREPARING, ORDER_READY]); 
                    })),
                    'total_amount' => array_sum(array_column($todayOrders, 'total'))
                ];
            }
            
            $this->view('waiters/table_orders', [
                'tableOrderSummary' => $tableOrderSummary,

...
class Table extends BaseModel {
    ...
    public function getUnassignedTablesWithOccupancy() {
        $query = "SELECT t.*, w.employee_code, u.name as current_waiter_name 
                  FROM {$this->table} t 
                  LEFT JOIN waiters w ON t.waiter_id = w.id 
                  LEFT JOIN users u ON w.user_id = u.id 
                  WHERE t.active = 1 AND (t.waiter_id IS NULL OR t.status != 'disponible')
                  ORDER BY t.number ASC";
    ...
    /**
     * Get tables available for a specific waiter based on locking rules:
     * - Available tables (can be taken by anyone)
     * - Tables already assigned to this waiter
     */
    public function getAvailableTablesForWaiter($waiterId) {
        $query = "SELECT * FROM {$this->table} 
                  WHERE active = 1 
                  AND (status = ? OR (status = ? AND waiter_id = ?))
                  ORDER BY number ASC";
        
        $stmt = $this->db->prepare($query);
        $stmt->execute([TABLE_AVAILABLE, TABLE_OCCUPIED, $waiterId]);
        
        return $stmt->fetchAll();
    }
    
    /**
     * Get all tables with waiter and reservation info for admin/cashier view
     */
    public function getTablesWithStatusInfo() {
        $query = "SELECT t.*, 
                         ...
                         CASE 
                             ...
                             ELSE 0 
                         END as has_upcoming_reservation,
                  ...
                  LEFT JOIN reservations r ON rt.reservation_id = r.id 
                      ...
                      AND r.reservation_datetime BETWEEN NOW() AND DATE_ADD(NOW(), INTERVAL 30 MINUTE)
                  WHERE t.active = 1 
                  ORDER BY t.number ASC";
